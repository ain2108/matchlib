...
//#define DEBUG 1
typedef cli_req_t<data32_t, SCRATCHPAD_ADDR_WIDTH, SCRATCHPAD_BANKS>
    tb_cli_req_t;
...
bool memmodel::check_response(tb_cli_rsp_t cli_rsp, tb_cli_req_t cli_req) {
  ...
  for (int i = 0; i < SCRATCHPAD_BANKS; i++) {
    if ((cli_req.valids[i] == true) &&
        (mem[cli_req.addr[i]] == cli_rsp.data[i]))
      pass_count++;
  }
...
SC_MODULE(TbIO) {
  ...
  // ANTON: The two LICs connecting to dut
  ...
  // ANTON: TODO: What is this for?
  Fifo fifo;

  void source();
  void sink();
  SC_HAS_PROCESS(TbIO);
  TbIO(sc_module_name name_)
      : sc_module(name_),
        clk("clk"),
        rst("rst"),
        cli_req("cli_req"),
...
void TbIO::source() {
  ...
  */
  // ANTON:  This is the request object itself
  curr_cli_req.opcode = STORE;

  DCOUT("=========================================================" << endl);
  DCOUT("@" << sc_time_stamp() << ":  Initializing addresses" << endl);
  // ANTON: SCRATCHPAD_CAPACITY/SCRATCHPAD_BANKS is capacity per bank?
  for (int i = 0; i < SCRATCHPAD_CAPACITY / SCRATCHPAD_BANKS; i++) {
    wait();
    // ANTON: Submit #banks requests
    for (int j = 0; j < SCRATCHPAD_BANKS; j++) {
      curr_cli_req.valids[j] = true;
      curr_cli_req.addr[j] =
          SCRATCHPAD_BANKS * i +
          j;  // ANTON: notice the addresses going to separate banks
      // curr_cli_req.data[j] = SCRATCHPAD_BANKS*i + j;
      curr_cli_req.data[j] = rand();
      DCOUT("@" << sc_time_stamp() << hex << ":  Lane " << j << ": addr 0x"
                << curr_cli_req.addr[j].to_uint64() << ", data 0x"
                << curr_cli_req.data[j].to_uint64() << endl);
    }
    // Send the store request to the DUT here:
    // ANTON: This submits all #banks requets to the scratchpad
  ...
  */
  ...
  for (int i = 0; i < SCRATCHPAD_CAPACITY / SCRATCHPAD_BANKS; i++) {
    ...
    cli_req.Push(curr_cli_req);
    // Push the requested load addresses into a fifo here for checking in the
    // sink function:
  ...
  DCOUT(" CPU TIME: " << (elapsed.user + elapsed.system) / 1e6 << " ms"
                      << std::endl);
  DCOUT(" SIMULATION SPEED: "
        << 1e9 * (sim_ns) / (elapsed.user + elapsed.system) << " Hz"
        << std::endl);
#endif
  ...
  while (1) {
    ...
    for (int j = 0; j < SCRATCHPAD_BANKS; j++) {
      if (curr_cli_rsp.valids[j] == true) {
        DCOUT("  Lane " << j << ": Requested addr 0x"
                        << ref_cli_req.addr[j].to_uint64()
                        << ", received data 0x"
                        << curr_cli_rsp.data[j].to_uint64() << endl);
      } else {
    ...
    if (!pass) {
      ...
      for (int j = 0; j < SCRATCHPAD_BANKS; j++) {
        DCOUT("\t\t   Expected data["
              << j << "]: " << refmem.read(ref_cli_req.addr[j]).to_uint64()
              << endl);
      }
      assert(false);
    } else {
...
SC_MODULE(TESTBENCH) {
  ...
  NVHLS_DESIGN(ScratchpadTop)
  dut;  // ANTON: Scratchpad instantiated here. Constructor defined elsewhere.
  TbIO TbIO_inst;  // ANTON: Instance of the TbIO here
  sc_clock clk;
  sc_signal<bool> rst;
  Connections::Combinational<tb_cli_req_t>
      cli_req;  // ANTON: The two LICs connecting TbIO and DUT
  ...
  SC_CTOR(TESTBENCH)
    ...
    // ANTON: Connecting the LICs to the Scratchpad
    dut.cli_req(cli_req);
    dut.cli_rsp(cli_rsp);

    TbIO_inst.clk(clk);
    TbIO_inst.rst(rst);
    // ANTON: Connecting the LICs to the TbIO
